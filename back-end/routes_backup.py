...
def get_activities_by_date(date):
    ...
    try:
        ...
        # Query per attività che iniziano in questa data
        activities_starting = Activity.query.filter_by(date=date_obj).all()
        
        # Query per attività multi-giorno che includono questa data
...
def get_activities_by_status(status):
    ...
    try:
        ...
        activities = Activity.query.filter_by(status=status).order_by(Activity.date.desc(), Activity.time.desc()).all()
        return jsonify([activity.to_dict() for activity in activities])
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def get_activity_stats():
    ...
    try:
        ...
        da_fare = Activity.query.filter_by(status=ActivityStatus.DA_FARE.value).count()
        in_corso = Activity.query.filter_by(status=ActivityStatus.IN_CORSO.value).count()
        fatta = Activity.query.filter_by(status=ActivityStatus.FATTA.value).count()
        rimandata = Activity.query.filter_by(status=ActivityStatus.RIMANDATA.value).count()
        
        # Statistiche per priorità
        alta_priority = Activity.query.filter_by(priority=ActivityPriority.ALTA.value).count()
        media_priority = Activity.query.filter_by(priority=ActivityPriority.MEDIA.value).count()
        bassa_priority = Activity.query.filter_by(priority=ActivityPriority.BASSA.value).count()
...
def get_categories():
    ...
    try:
        categories = db.session.query(Activity.category).filter(Activity.category.isnot(None)).distinct().all()
